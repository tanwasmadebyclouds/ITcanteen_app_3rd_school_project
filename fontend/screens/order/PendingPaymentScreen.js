...
const PendingPaymentScreen = ({navigation}) => {
  ...
  function refresh(){
    ...
    <View style={styles.container}>
      ...
      {data.filter(orders=>orders.status === "payment").length === 0 &&
      ...
      {data.filter(orders=>orders.status === "payment").length === 0 &&
        <Text style={{marginTop:"50%", fontWeight:"bold", fontSize:18}}>ไม่มีคำสั่งซื้อ</Text>
          ...
          {!loading && (
                <OrderList navigation={navigation} listData={data.filter(orders=>orders.status === "payment")} button={"payment"}/>
            )}
