...
const BlackListScreen = ({navigation}) => {
  ...
  <View style={styles.container}>
      ...
      {data.filter(orders=>orders.status === "blacklist").length === 0 &&
      ...
      {data.filter(orders=>orders.status === "blacklist").length === 0 &&
        ...
        {!loading && (
              <OrderList navigation={navigation} listData={data.filter(orders=>orders.status === "blacklist")} button={"blacklist"}/>
          )}
  </View>
  );
