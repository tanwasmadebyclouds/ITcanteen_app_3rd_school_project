...
const radioButtonsMeatData = [{
  id: '1', // acts as primary key, should be unique and non-empty string
  label: <Text style={{fontSize:15}}>25%</Text>,
  value: '25%',
  selected: true,
}, {
  id: '2',
  label:<Text style={{fontSize:15}}>50%</Text>,
  value: '50%',
  selected: false,
}, {
  id: '3',
  label: <Text style={{fontSize:15}}>75%</Text>,
  value: '75%',
  selected: false,
}, {
  id: '4',
  label: <Text style={{fontSize:15}}>100%</Text>,
  value: '100%',
  selected: false,
}]
...
const App = ({navigation, route, Name, Price, uri, shop_id, back}) => {
  ...
  const addCart = () =>{
    ...
    if (describtionType == 'เล็ก') {
      cart.push({
        id:uuid.v4(),
        name:Name,
        price: Price,
        details:addData+" "+ describtionType + ','+"ความหวาน "+" "+describtionmeat,
        quantity: quantity,
        shop_id: shop_id,
        uri: uri,
    ...
    <ScrollView>
      ...
      <View style={styles.detailcontainner}>
        <View style={styles.radioContainners}>
            ...
            <Text style={styles.title}>ระดับความหวาน</Text>
            <View style={styles.radio}>
              <RadioGroup 
            ...
            numberOfLines={6}
            ></TextInput>
            {onPressRadioButtonvaleType() == 'เล็ก' &&
            <Text style={styles.title}>ราคา:      {Price} บาท</Text>
            }

            {
              onPressRadioButtonvaleType() == 'ใหญ่' &&
